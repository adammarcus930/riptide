...
1. **Weekly targets.** Per muscle: the **midpoint** of the effort range (floored at 2). Distribution across days is handled separately (step 5), so the target does not depend on the day count. **Correction (2026-07-20):** earlier drafts nudged the target within the range to divide evenly across the day count; once distribution moved to a concentrate-at-~3 model that handles any total, that nudge only made volume swing arbitrarily with day count (e.g. chest 10 at 5 days, 14 at 7) — so the target is now simply the midpoint.
2. **Secondary credit — fixed order, one pass.** Giver muscles (chest, lats, front delts, side delts, rear delts, traps, quads, hamstrings, calves) are allocated at full target first. Their exercises' secondary contributions are tallied at **0.5 credit per set**; receiver muscles (triceps, biceps, forearms, abs) then aim their direct work at `midpoint − credit`. **A selected receiver is guaranteed at least one real direct appearance (2 sets)** whenever there is still room under the top of its range (`range.high − credit ≥ 2`) — so a muscle you explicitly chose gets some direct work unless pressing/pulling has already filled it to the top of its range. Direct work only reaches 0 when credit alone meets or exceeds `range.high`. No iteration, no circular math.
3. **Rotation.** Each muscle's weekly sets split into per-day appearances; the user's chosen exercises for that muscle rotate round-robin across appearances. Fewer exercises than appearances → cycle; more → used as evenly as possible within the week.
...
5. **Concentrate, don't spread.** A muscle appears on ≈`target ÷ 3` days at ~3 sets per appearance, not on every available day at 2 sets. This deliberately minimizes lifts per day and keeps each exercise at a productive set count, rather than maximizing frequency. **Correction (2026-07-20):** earlier drafts spread each muscle across as many days as possible (flooring at 2 sets/appearance), which produced ~8 short lifts/day on high-day-count full-body programs — the opposite of how the user actually trains. The number of days a muscle appears on is raised only if the per-day capacity cap (`exerciseCount × 4`) would otherwise be exceeded. Appearances are staggered so daily set totals stay level.
